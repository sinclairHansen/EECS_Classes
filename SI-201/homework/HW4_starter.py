...
# Your name: Sinclair Hansen
# Your student id: 44136504
# Your email: sihansen@umich.edu
...
"""
Used ChatGPT for debugging help with the checkout function, as well as general syntax for classes.
It does align with my goals for the contract as I used it to help with concepts I was struggling with
"""
...
class Library:
    ...
    def checkout_book(self, patron_id, book_id):
        ...
        '''
        # ==============================        
        # Does the patron exist in the library list?
        # Does patron exist in library?
        p = None
        for patron in self.patrons:
            if patron_id == patron.patron_id:
                p = patron
                break
        b = None
        ...
        if len(p.checked_out_books) >= 3:
            self.transaction_log.append((patron_id, book_id, "checkout", False))
            return False
        

        p.checked_out_books.append(b)
        b.is_checked_out = True
        b.checkout_history.append(p)
    ...
    def return_book(self, patron_id, book_id):
        ...
        # Is the book currently checked out with the patron?
        if b not in p.checked_out_books:
            self.transaction_log.append((patron_id, book_id, "return", False))
            return False
                
        # IF the return is sucessful
        p.checked_out_books.remove(book)
        ...
        # If the return is not sucessful 
        # do not modify the patron or book state
        # record the transaction as unsucessful
        # ==============================
        # YOUR CODE ENDS HERE
        # ==============================
    ...
    def count_successful_checkouts(self):
        ...
        '''
        sucessful_trans = {}
        for p_id, b_id, action, result in self.transaction_log:
            if action == "checkout" and result:
                sucessful_trans[b_id] = sucessful_trans.get(b_id, 0) + 1
        return sucessful_trans
